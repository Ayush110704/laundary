...
const AdminLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        // HIT THE LIVE BACKEND LOGIN ENDPOINT
        ...
        if (data.success) {
          console.log('Admin logged in successfully via MongoDB', data);
          
          // Store authentication signature and info details for authorization checks
          const sessionData = {
            id: data.admin.id,
          ...
          // Keep persistent session metadata intact for your Dashboard UI component state checks
          localStorage.setItem('adminSession', JSON.stringify(sessionData));
          localStorage.setItem('adminToken', data.token);
          
          // Redirect to administrative control layout workspace
          navigate('/admin-dashboard');
        } else {
          // Display mismatch error responses passed up directly from database security layers
          setLoginError(data.message || 'Invalid email or password');
        }
      } catch (error) {
        console.error('Login system processing exception:', error);
        setLoginError('Could not reach the database authentication server. Ensure port 5000 is active!');
      } finally {
        setIsLoading(false);
      }
    }
